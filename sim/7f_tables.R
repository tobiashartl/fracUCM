...
for(i in 1:nrow(setups)){
    true.par          <- setups[i, ]
    try({
        load(paste("./MC/MC_7/CSS/Sim_R1000_n", true.par[1], "_d", true.par[2], "_r", true.par[3],
                   "_corr0_trendbreak.RData", sep=""))
...
for(i in 1:nrow(setups)){
    true.par          <- setups[i, ]
    try({
        load(paste("./MC/MC_7/ML/Sim_R1000_n", true.par[1], "_d", true.par[2], "_r", true.par[3],
                   "_corr0_trendbreak.RData", sep=""))
        results.list[[i]] <- RESULTS
        nu <- nucalc(true.par$n, true.par$d, true.par$ratio, ar)
...
for(i in 1:nrow(setups)){
    true.par          <- setups[i, ]
    try({
        load(paste("./MC/MC_7/CSS_INTEGER/Sim_R1000_n", true.par[1], "_d", true.par[2], "_r", true.par[3],
                   "_trendbreak.RData", sep=""))
        results.list[[i]] <- RESULTS
        nu <- nucalc(true.par$n, true.par$d, true.par$ratio, ar)
...
for(i in 1:nrow(setups)){
    true.par          <- setups[i, ]
    try({
        load(paste("./MC/MC_7/ML_INTEGER/Sim_R1000_n", true.par[1], "_d", true.par[2], "_r", true.par[3],
                   "_trendbreak.RData", sep=""))
        results.list[[i]] <- RESULTS
        nu <- nucalc(true.par$n, true.par$d, true.par$ratio, ar)
        SSR   <- mean(results.list[[i]][c("SSR"), ])
...
for(i in 1:nrow(setups)){
    true.par          <- setups[i, ]
    try({
        load(paste("./MC/MC_7/ARMA/Sim_R1000_n", true.par[1], "_d", true.par[2], "_r", true.par[3],
                   "_corr0_trendbreak.RData", sep=""))
        results.list[[i]] <- RESULTS
        nu <- nucalc(true.par$n, true.par$d, true.par$ratio, ar)
...
saveRDS(tabjoint, file = "./MC/MC_7/tab1_biasrmse_d.RDS")
...
tab3$r[tab3$r==0]<-NA
saveRDS(tab3, file = "./MC/MC_7/tab2_rmse_oth.RDS")
...
tab3$r[!c(1:36)%in%seq(1, 36, by = 3)] <- NA
tab3$r[tab3$r==0]<-NA
saveRDS(tab3, file = "./MC/MC_7/tab3_bias_oth.RDS")
...
# CSS
load(file = "./MC/MC_7/CSS/results.RData")
X.hat.CSS <- X.res
C.hat.CSS <- C.res
# ML
load(file = "./MC/MC_7/ML/results_ML.RData")
X.hat.ML <- X.res
C.hat.ML <- C.res
# CSS i1
load(file = "./MC/MC_7/CSS_INTEGER/results_i1.RData")
X.hat.i1 <- X.res
C.hat.i1 <- C.res
# CSS i1 ML
load(file = "./MC/MC_7/ML_INTEGER/results_i1_ML.RData")
X.hat.i1.ML <- X.res
C.hat.i1.ML <- C.res
# ARMA ML
load(file = "./MC/MC_7/ARMA/results_ARMA.RData")
X.hat.ARMA <- X.res
C.hat.ARMA <- C.res
...
calcRSQ_x <- function(x.true, x.est){
    Rsq <- mean(sapply(1:ncol(x.true), function(j) summary(lm(x.true[, j] ~ I(1:NROW(x.true)) + x.est[,j]))$r.squared))
}
calcRSQ_c <- function(x.true, x.est){
    Rsq <- mean(sapply(1:ncol(x.true), function(j) summary(lm(x.true[, j] ~ x.est[,j]))$r.squared))
}

Rsq.CSS <- sapply(1:nrow(setups), function(j) calcRSQ_x(X.true[[j]], X.hat.CSS[[j]]))
Rsq.ML <- sapply(1:nrow(setups), function(j) calcRSQ_x(X.true[[j]], X.hat.ML[[j]]))
Rsq.i1 <- sapply(1:nrow(setups), function(j) calcRSQ_x(X.true[[j]], X.hat.i1[[j]]))
Rsq.i1.ML <- sapply(1:nrow(setups), function(j) calcRSQ_x(X.true[[j]], X.hat.i1.ML[[j]]))
Rsq.ARMA <- sapply(1:nrow(setups), function(j) calcRSQ_x(X.true[[j]], X.hat.ARMA[[j]]))
Rsq.CSS.C <- sapply(1:nrow(setups), function(j) calcRSQ_c(C.true[[j]], C.hat.CSS[[j]]))
Rsq.ML.C <- sapply(1:nrow(setups), function(j) calcRSQ_c(C.true[[j]], C.hat.ML[[j]]))
Rsq.i1.C <- sapply(1:nrow(setups), function(j) calcRSQ_c(C.true[[j]], C.hat.i1[[j]]))
Rsq.i1.ML.C <- sapply(1:nrow(setups), function(j) calcRSQ_c(C.true[[j]], C.hat.i1.ML[[j]]))
Rsq.ARMA.C <- sapply(1:nrow(setups), function(j) calcRSQ_c(C.true[[j]], C.hat.ARMA[[j]]))
...
saveRDS(tab4, file = "./MC/MC_7/tab4_rsq.RDS")
