...
for ( i in 1:NROW(setups)){
    ...
    if(file.exists(file = paste("./MC/MC_6/ML_INTEGER/Sim_R", R, "_n", n, "_d", d, "_r", r, "_trend.RData", sep=""))) next
    set.seed(42)
        ...
        tryCatch({
            grid.st <- cbind(gr.start, 
                             sapply(1:nrow(gr.start), 
                                    function(i) UC_opt_KF_i1(gr.start[i, ],  deterministics = "trend",
                                                             y=y, START = 1, ll=TRUE,
                                                             nulim = nulim)))
    ...
    save(RESULTS, file = paste("./MC/MC_6/ML_INTEGER/Sim_R", R, "_n", n, "_d", d, "_r", r, "_trend.RData", sep=""))
}
