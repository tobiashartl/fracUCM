...
for ( i in 1:NROW(setups)){
    ...
    if(file.exists(file = paste("./MC/MC_5/ML_INTEGER/Sim_R", R, "_n", n, "_d", d, "_r", r, "_outlier.RData", sep=""))) next
    set.seed(42)
        ...
        tryCatch({
            grid.st <- cbind(gr.start, 
                             sapply(1:nrow(gr.start), 
                                    function(i) UC_opt_KF_i1(gr.start[i, ],  deterministics = F,
                                                             y=y, START = 1, ll=TRUE,
                                                             nulim = nulim)))
            
            par0 <- grid.st[which.min(grid.st[,4]),-4]
            est <- optim(par=par0, deterministics = F,
    ...
    save(RESULTS, file = paste("./MC/MC_5/ML_INTEGER/Sim_R", R, "_n", n, "_d", d, "_r", r, "_outlier.RData", sep=""))
}
