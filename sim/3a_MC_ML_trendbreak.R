...
for ( i in 1:nrow(setups)){
    ...
    if(file.exists(file = paste("./MC/ML/Sim_R", R, "_n", n, "_d", d, "_r", r, "_corr0_trendbreak.RData", sep=""))) next
    set.seed(42)
    ...
    dEW_45 <- c(apply(y, 2, EW, type = 3, interval = c(0, 2), alpha = .45))
    dEW_50 <- c(apply(y, 2, EW, type = 3, interval = c(0, 2), alpha = .50))
    dEW_55 <- c(apply(y, 2, EW, type = 3, interval = c(0, 2), alpha = .55))
    dEW_60 <- c(apply(y, 2, EW, type = 3, interval = c(0, 2), alpha = .60))
    dEW_65 <- c(apply(y, 2, EW, type = 3, interval = c(0, 2), alpha = .65))
    dEW_70 <- c(apply(y, 2, EW, type = 3, interval = c(0, 2), alpha = .70))
    ...
    save(RESULTS, file = paste("./MC/ML/Sim_R", R, "_n", n, "_d", d, "_r", r, "_corr0_trendbreak.RData", sep=""))
}
