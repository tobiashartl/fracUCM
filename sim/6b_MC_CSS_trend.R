...
for ( i in 1:NROW(setups)){
    ...
    if(file.exists(file = paste("./MC/MC_6/CSS/Sim_R", R, "_n", n, "_d", d, "_r", r, "_corr0_trend.RData", sep=""))) next
    set.seed(42)
    ...
    trend <- (1/1) * (1:n) # linearly increasing trend
    ...
    optfn <- function(n, y, x){
        tryCatch({
            # Estimate
            # check grid
            grid.st <- cbind(gr.start, sapply(1:nrow(gr.start), function(i) fUC_opt(gr.start[i, ], deterministics = "trend",
                                                                                    y=y, START = 2,
                                                                                    nulim = nulim)))
            ...
            (est <- optim(par=par0, control = list(),
                          fn = fUC_opt, quiet = TRUE, START = 2, deterministics = "trend",
            ...
            while(est$par[1] < .55 | est$par[1] > 2.45 | exp(est$par[2]) < 0.001 | exp(est$par[2]) > 250000){
                ...
                (est <- optim(par=par0, 
                              ...
                              y=y, method = "BFGS", nulim = nulim, deterministics = "trend",
                              START = 2))
            ...
            if(is.na(est$par[1])  |  est$par[1] < .55 | est$par[1] > 2.45){
                ...
                if(is.na(est$par[1]) |  est$par[1] < .55 | est$par[1] > 2.45){
                    ...
                    est <- tryCatch({
                        (optim(par=par0, 
                               fn = fUC_opt, quiet = TRUE, d.int = c(0.5, 2.5), deterministics = "trend",
            ...
            Rsq <- summary(lm(x ~ KS$x + I(1:n)))$r.squared
    ...
    ### fUC part
    cl <- makeCluster(32)
    clusterExport(cl, c("optfn", "fUC_opt", "y", "fUC_comp", "ma_inf", "nulim", 
    ...
    dEW_45 <- c(apply(y, 2, EW, type = 3, interval = c(0, 2.5), alpha = .45))
    dEW_50 <- c(apply(y, 2, EW, type = 3, interval = c(0, 2.5), alpha = .50))
    dEW_55 <- c(apply(y, 2, EW, type = 3, interval = c(0, 2.5), alpha = .55))
    dEW_60 <- c(apply(y, 2, EW, type = 3, interval = c(0, 2.5), alpha = .60))
    dEW_65 <- c(apply(y, 2, EW, type = 3, interval = c(0, 2.5), alpha = .65))
    dEW_70 <- c(apply(y, 2, EW, type = 3, interval = c(0, 2.5), alpha = .70))
    ...
    save(RESULTS, file = paste("./MC/MC_6/CSS/Sim_R", R, "_n", n, "_d", d, "_r", r, "_corr0_trend.RData", sep=""))
}
