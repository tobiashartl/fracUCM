...
for ( i in 1:nrow(setups)){
    ...
    if(file.exists(file = paste("./MC/MC_7/ML/Sim_R", R, "_n", n, "_d", d, "_r", r, "_corr0_trendbreak.RData", sep=""))) next
    set.seed(42)
    ...
    optfn <- function(n, y, x){
        tryCatch({
            ...
            (est <- optim(par=par0, control = list(), nu.opt=TRUE,
                          fn = fUC_opt_ML, quiet = TRUE, START = 2,
    ...
    save(RESULTS, file = paste("./MC/MC_7/ML/Sim_R", R, "_n", n, "_d", d, "_r", r, "_corr0_trendbreak.RData", sep=""))
}
