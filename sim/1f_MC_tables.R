...
for(i in 1:nrow(setups)){
    true.par          <- setups[i, ]
    try({
        load(paste("./MC/MC_1/CSS/Sim_R1000_n", true.par[1], "_d", true.par[2], "_r", true.par[3],
                   "_corr0.RData", sep=""))
...
for(i in 1:nrow(setups)){
    true.par          <- setups[i, ]
    try({
        load(paste("./MC/MC_1/ML/Sim_R1000_n", true.par[1], "_d", true.par[2], "_r", true.par[3],
                   "_corr0.RData", sep=""))
        results.list[[i]] <- RESULTS
        nu <- nucalc(true.par$n, true.par$d, true.par$ratio, ar)
...
for(i in 1:nrow(setups)){
    true.par          <- setups[i, ]
    try({
        load(paste("./MC/MC_1/CSS_INTEGER/Sim_R1000_n", true.par[1], "_d", true.par[2], "_r", true.par[3],
                   "_corr0.RData", sep=""))
        results.list[[i]] <- RESULTS
        nu <- nucalc(true.par$n, true.par$d, true.par$ratio, ar)
...
for(i in 1:nrow(setups)){
    true.par          <- setups[i, ]
    try({
        load(paste("./MC/MC_1/ML_INTEGER/Sim_R1000_n", true.par[1], "_d", true.par[2], "_r", true.par[3],
                   "_corr0.RData", sep=""))
        results.list[[i]] <- RESULTS
        nu <- nucalc(true.par$n, true.par$d, true.par$ratio, ar)
        SSR   <- mean(results.list[[i]][c("SSR"), ])
...
for(i in 1:nrow(setups)){
    true.par          <- setups[i, ]
    try({
        load(paste("./MC/MC_1/ARMA/Sim_R1000_n", true.par[1], "_d", true.par[2], "_r", true.par[3],
                   "_corr0.RData", sep=""))
        results.list[[i]] <- RESULTS
        nu <- nucalc(true.par$n, true.par$d, true.par$ratio, ar)
...
saveRDS(tabjoint, file = "./MC/MC_1/tab1_biasrmse_d.RDS")

tabjoint[abs(tabjoint)>100 & !is.na(tabjoint>100)] <- round(tabjoint[abs(tabjoint) >100 & !is.na(tabjoint>100)], digits = 0)

table <- tabjoint%>% 
    xtable::xtable(digits = c(0, 0, 0, 2, 2, rep(2, 12)), include.rownames=FALSE)
print(table, include.rownames = FALSE)
...
tab3$r[tab3$r==0]<-NA
saveRDS(tab3, file = "./MC/MC_1/tab2_rmse_oth.RDS")
...
tab3$r[!c(1:36)%in%seq(1, 36, by = 3)] <- NA
tab3$r[tab3$r==0]<-NA
saveRDS(tab3, file = "./MC/MC_1/tab3_bias_oth.RDS")
...
# CSS
load(file = "./MC/MC_1/CSS/results.RData")
X.hat.CSS <- X.res
C.hat.CSS <- C.res
# ML
load(file = "./MC/MC_1/ML/results_ML.RData")
X.hat.ML <- X.res
C.hat.ML <- C.res
# CSS i1
load(file = "./MC/MC_1/CSS_INTEGER/results_i1.RData")
X.hat.i1 <- X.res
C.hat.i1 <- C.res
# CSS i1 ML
load(file = "./MC/MC_1/ML_INTEGER/results_i1_ML.RData")
X.hat.i1.ML <- X.res
C.hat.i1.ML <- C.res
# ARMA ML
load(file = "./MC/MC_1/ARMA/results_ARMA.RData")
X.hat.ARMA <- X.res
C.hat.ARMA <- C.res
...
saveRDS(tab4, file = "./MC/MC_1/tab4_rsq.RDS")


tab4[tab4>100 & !is.na(tab4>100)] <- round(tab4[tab4>100& !is.na(tab4>100)], digits = 0)
table <- tab4%>% 
