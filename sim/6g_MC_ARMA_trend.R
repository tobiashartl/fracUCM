...
for ( i in 1:nrow(setups)){
    ...
    r <- setups[i, 3]
    cat("Iteration ", i, "\n")
    #if(file.exists(file = paste("./MC/MC_6/ARMA/Sim_R", R, "_n", n, "_d", d, "_r", r, "_corr0_trend.RData", sep=""))) next
    set.seed(42)
    ...
    dEW_45 <- NA#c(apply(y, 2, EW, type = 3, interval = c(0, 2.5), alpha = .45))
    dEW_50 <- NA#c(apply(y, 2, EW, type = 3, interval = c(0, 2.5), alpha = .50))
    dEW_55 <- NA#c(apply(y, 2, EW, type = 3, interval = c(0, 2.5), alpha = .55))
    dEW_60 <- NA#c(apply(y, 2, EW, type = 3, interval = c(0, 2.5), alpha = .60))
    dEW_65 <- NA#c(apply(y, 2, EW, type = 3, interval = c(0, 2.5), alpha = .65))
    dEW_70 <- NA#c(apply(y, 2, EW, type = 3, interval = c(0, 2.5), alpha = .70))
    ...
    save(RESULTS, file = paste("./MC/MC_6/ARMA/Sim_R", R, "_n", n, "_d", d, "_r", r, "_corr0_trend.RData", sep=""))
}
