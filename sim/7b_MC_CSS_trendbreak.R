...
for ( i in 1:NROW(setups)){
    ...
    if(file.exists(file = paste("./MC/MC_7/CSS/Sim_R", R, "_n", n, "_d", d, "_r", r, "_corr0_trendbreak.RData", sep=""))) next
    set.seed(42)
    ...
    optfn <- function(n, y, x){
        tryCatch({
            # Estimate
            # check grid
            grid.st <- cbind(gr.start, sapply(1:nrow(gr.start), function(i) fUC_opt(as.numeric(gr.start[i, ]), 
                                                                                    y=y, START = 2,
                                                                                    nulim = nulim,
    ...
    save(RESULTS, file = paste("./MC/MC_7/CSS/Sim_R", R, "_n", n, "_d", d, "_r", r, "_corr0_trendbreak.RData", sep=""))
}
