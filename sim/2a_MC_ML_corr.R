...
for ( i in 1:NROW(setups)){
    ...
    r <- setups[i, 3]
    cat("Iteration ", i, "\n")
    if(file.exists(file = paste("./MC/MC_2/ML/Sim_R", R, "_n", n, "_d", d, "_r", r, "_corr.RData", sep=""))) next
    set.seed(42)
    ...
    optfn <- function(n, y, x){
        tryCatch({
            ...
            est <- tryCatch({
                (optim(par=par0, control = list(), corr=TRUE,
            ...
            # check for corner solutions: d->0.5, d->2.5, nu->1/100, nu->100
            j=1
            while(is.na(est$par[1])  |  est$par[1] < .55 | est$par[1] > 2.45){
                if(j > 10) break # further starting values
                j=j+1
    ...
    save(RESULTS, file = paste("./MC/MC_2/ML/Sim_R", R, "_n", n, "_d", d, "_r", r, "_corr.RData", sep=""))
}
