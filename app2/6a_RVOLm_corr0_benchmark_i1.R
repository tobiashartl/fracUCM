...
for(p in c(1:12)){
    ...
    optfn <- function(par0, y){ tryCatch({
        est <- optim(par=par0, method = "BFGS", 
                     fn = UC_opt_ML, deterministics = FALSE,  
    ...
    RESULTS <- t(RESULTS)
    save(RESULTS, file = paste("./app2/Benchmark_ML_i1_p", p, "_none_corr0.RData", sep=""))
...
for(p in c(1:12)){
    cat("p = ", p, "\n")
    load(file = paste("./app2/Benchmark_ML_i1_p", p, "_none_corr0.RData", sep=""))
...
TC <- fUC_KS_ARMA_approx(c(1, theta), y , nulim = c(0, Inf), 
...
colnames(EST)<- c("par", "se")
saveRDS(EST, file = "./app2/Benchmark_i1_ML_Results.RDS")
...
saveRDS(data.plot, file = "./app2/Benchmark_i1_ML_TC.RDS")
plot(data.plot$y)
lines(data.plot$x)
