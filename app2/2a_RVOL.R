...
# load data
download.file("https://cdn.cboe.com/api/global/us_indices/daily_prices/VIX_History.csv",
              "./app_2/VIX.csv")

data<- read.csv(file = "./app_2/VIX.csv", skip = 0)[, ] %>%
    as.data.frame() %>%
    mutate(DATE = as.Date(DATE, format = "%m/%d/%Y")) %>%
    mutate(lClose = log(CLOSE))

plot((data$lClose) ~ data$DATE, type="l")
...
c(log(var(frac_diff(y, 2))), log(var(frac_diff(y, 1))), 
                                             # variance in fractional differences: about -5
  log(var(frac_diff(y, 0.5))), log(var(y)))  # -2: maximum variance to sample from 
...
for(i in 1:nrow(grid)){
    ...
    save(RESULTS, file = paste("./app2/2b_VIX_", det, "_p", p, "_noseas_diff.RData", sep=""))
